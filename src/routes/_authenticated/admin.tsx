...
function Admin() {
  ...
  const createChallenge = useMutation({
    mutationFn: async (input: { title: string; description: string; category: string; target_kg: number }) => {
      const { error } = await supabase.from("challenges").insert({ ...input, created_by: userId, active: true });
      if (error) throw error;
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["admin-challenges"] }); toast.success("Challenge created"); },
    ...
    return (
      <GlassCard className="max-w-md mx-auto text-center">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          You need the <code className="px-1 py-0.5 bg-muted rounded">admin</code> role.
          Grant it via the Cloud Database for your user in <code>user_roles</code>.
        </p>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Manage users, challenges, and community posts.</p>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <GlassCard>
          ...
          <form
            onSubmit={(e) => {
              e.preventDefault();
              createChallenge.mutate({ title, description: desc, category: cat, target_kg: target });
              setTitle(""); setDesc("");
            }}
            className="space-y-3"
          >
            <div><Label htmlFor="t">Title</Label><Input id="t" value={title} onChange={(e) => setTitle(e.target.value)} required /></div>
            <div><Label htmlFor="d">Description</Label><Textarea id="d" value={desc} onChange={(e) => setDesc(e.target.value)} required /></div>
            <div className="grid grid-cols-2 gap-3">
              <div><Label htmlFor="c">Category</Label><Input id="c" value={cat} onChange={(e) => setCat(e.target.value)} /></div>
              <div><Label htmlFor="tk">Target kg COâ‚‚e</Label><Input id="tk" type="number" value={target} onChange={(e) => setTarget(Number(e.target.value))} /></div>
            </div>
        ...
        <GlassCard>
          <h2 className="font-display font-semibold mb-3">Challenges ({challengesQ.data?.length ?? 0})</h2>
          <ul className="divide-y divide-border/40 max-h-80 overflow-y-auto">
            {challengesQ.data?.map((c) => (
              <li key={c.id} className="flex items-center gap-3 py-2">
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-sm truncate">{c.title}</p>
                  <p className="text-xs text-muted-foreground">{c.category} · {c.active ? "active" : "inactive"}</p>
                </div>
                <Button size="sm" variant="ghost" onClick={() => toggleChallenge.mutate({ id: c.id, active: !c.active })}>
          ...
          <div className="max-h-80 overflow-y-auto">
            <table className="w-full text-sm">
              <thead className="text-left text-xs text-muted-foreground">
                <tr><th className="py-2">Name</th><th>Score</th><th>Streak</th><th>Joined</th></tr>
              </thead>
              <tbody>
                {usersQ.data?.map((u) => (
